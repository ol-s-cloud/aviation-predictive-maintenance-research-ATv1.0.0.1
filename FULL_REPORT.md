...
```mermaid
flowchart TD
  A["Aircraft OEMs: Airbus, Boeing, Embraer"]
  B["OEM Digital Ecosystems: Skywise, Insight Accelerator, IntelligentEngine, GE Maintenance Insight"]
  C["Independent Airline/MRO Platforms: AVIATAR, Prognos, Smart MRO"]
  D["ERP/CMMS Suites: AMOS, Ramco, IFS Maintenix, EmpowerMX, CAMP (Veryon)"]
  E["Specialist Analytics: Honeywell Forge, ACSIS, Uptake"]
  X["AirTwin: Environmental Exposure Twin + PdM\nPrescriptive Workcards + ROI Tracker"]
  G["Climate & Sustainability: Satavia NETZERO, Atmosfair, Climate X"]
  F["Inspection & Robotics: Donecle, Mainblades, Waygate Technologies"]
  H["Systems Integrators / Resellers: Accenture, SITA, Capgemini, IBM"]
  I["End Users: Airlines, MROs, Lessors, Regulators"]
  ...
  classDef airtwin fill:#d1ffd8,stroke:#2e7d32,stroke-width:2px;
  class X airtwin;
```
...
```mermaid
flowchart LR
    A[Aircraft Sensors<br/>Flight/QAR/FDR Data] --> B[OEM / Platform<br/>Data Hubs]
    B --> C[ERP/CMMS<br/>AMOS/Ramco/IFS/EmpowerMX]
    C --> D[PdM Engines<br/>ACSIS/FORGE + Exposure/ESG]
    D --> E[Reliability Engineer<br/>& Planner]
    E --> F[Workcard + Parts Order<br/>+ Slot Booking]
    F --> G[Line/Base Execution<br/>Inspection/Robotics Feeds]
    G --> H[Return to Service]
    H --> I[Finance & ESG<br/>Reporting · Feedback Loop]
    I -.Continuous Improvement.-> B
...
```mermaid
flowchart TD
    OS[OpenSky Network<br/>ADS-B Operations & Trajectory]:::dataset --> X[Exposure Model<br/>salt/particulate/humidity]
    ERA[ERA5 Reanalysis<br/>Weather: temp/humidity/winds]:::dataset --> X
    CAMS[CAMS EAC4<br/>Aerosol Data: sea-salt/sulfate]:::dataset --> X
    
    X --> RS[Risk Score<br/>per tail/route]
    RS --> PR[Prescriptions<br/>wash/inspect windows + ROI]
    PR --> CM[CMMS/ERP<br/>Automated Workcards]
    CM --> EX[Execution & Inspection<br/>drones/borescope data]
    EX --> FB[Feedback Loop<br/>Model Refinement]
    FB -.Continuous Learning.-> X
    
    classDef dataset fill:#e3f2fd,stroke:#1976d2,stroke-width:2px
    style X fill:#fff3e0
